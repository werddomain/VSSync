...
- **Keyboard Shortcut**: Quick access with `Ctrl+Shift+V, S` on both IDEs
...
3. **OR use the keyboard shortcut**: `Ctrl+Shift+V, S` (press `Ctrl+Shift+V`, then press `S`)
   - On macOS: `Cmd+Shift+V, S`
